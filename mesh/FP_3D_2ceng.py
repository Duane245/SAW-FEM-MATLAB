...
def GM(pitch, n_idt, Aperture, eta, Hidt, Hidt_LT,Hidt_LT2, Hidt_PML, n_f, n_we, msh_max, msh_min, msh_r, msh_f):
    gmsh.initialize()

    gmsh.model.add("FP_3D_2ceng")
   ...
   # 增加PML线
    ...
    complement_pml_left2_tags = []  # pml体 左2 下
    complement_pml_right_tags = []  # pml体 右0 上
    complement_pml_right1_tags = []  # pml体 右1 中
    complement_pml_right2_tags = []  # pml体 右2 下
   ...
   # complement_side_tags = []  # side体
    ...
    for t in range(1, N_IDT + N_IDT + 4):
        ...
        # 拉伸pml
        ov3 = gmsh.model.geo.extrude([ov2[0]], 0, 0, -h_pml, [NN_PML], [], True)

        # 电极+电极间的面
        if t < (2 * N_IDT + 2):
            complement_pe_tags.append(ov[1][1])
            complement_pe2_tags.append(ov2[1][1])
            complement_pml_low_tags.append(ov3[1][1])
        ...
        boundary_bottom_tags.append(ov3[0][1])
...
pitch = np.array([1.085, 1.085, 1.085])  # 左悬浮电势 中间IDT 右悬浮电势
n_idt = np.array([6, 25, 6])             # 对应根数
...
msh = GM(pitch, n_idt, 2, eta, 0.2, 0.6, 7.4,1.085*2, n_f, n_we, 0, 0.3, 0, 0)
