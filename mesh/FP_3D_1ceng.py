...
def GM(pitch, n_idt, Aperture, eta, Hidt, Hidt_LT, Hidt_PML, n_f, n_we, msh_max, msh_min, msh_r, msh_f):
    gmsh.initialize()

    gmsh.model.add("FP_3D_1ceng")
   ...
   # 增加PML线
    ...
    complement_pml_left_tags = []  # pml体 左上
    complement_pml_left1_tags = []  # pml体 左下
    complement_pml_right_tags = []  # pml体 右上
    complement_pml_right1_tags = []  # pml体 右下
    ...
    for t in range(1, N_IDT + N_IDT + 4):
        ...
        # 拉伸pml
        ov2 = gmsh.model.geo.extrude([ov[0]], 0, 0, -h_pml, [NN_PML], [], True)
        # 电极+电极间的面
        if t < (2 * N_IDT + 2):
        ...
        boundary_bottom_tags.append(ov2[0][1])
...
msh = GM(pitch, n_idt, 2, eta, 0.2, 8, 2, n_f, n_we, 0, 0.15, 0, 0)
