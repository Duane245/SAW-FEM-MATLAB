...
def build_mesh(config):
    """SAW_3D_Hex9_LTPML_3ceng: Gmsh 模型构建(从 MATLAB 仓库 .py 复制并函数化)。"""
    gmsh.initialize()

    gmsh.model.add("SAW_3D_Hex9_LTPML_3ceng")
    ...
    NN2 = max(1, math.ceil(3 * h2 / 0.5 * 0.217 / lc - 1e-12))
    NN3 = max(1, math.ceil(30 * h3 / 6.51 * 0.217 / lc - 1e-12))
    NN4 = max(1, math.ceil(11 * h4 / 2.17 * 0.217 / lc - 1e-12))
