...
def build_mesh(config):
    """SAW_3D_Hex8_LTPML: Gmsh 模型构建(从 MATLAB 仓库 .py 复制并函数化)。"""
    gmsh.initialize()

    gmsh.model.add("SAW_3D_Hex8_LTPML")
    ...
    h2 = config.layers[0].thickness_um
    h3 = 2 * p
    h = h1 + h2 + h3  # 压电材料高度
    ...
    NN1 = max(1, math.ceil(3 * h1 / 0.6 * 0.217 / lc - 1e-12))
    NN2 = max(1, math.ceil(30 * h2 / 6.51 * 0.217 / lc - 1e-12))
    NN3 = max(1, math.ceil(10 * h3 / 2.17 * 0.217 / lc - 1e-12))
    ...
    gmsh.model.addPhysicalGroup(2, [37, 191, 301], 1)  # 左平面
    gmsh.model.addPhysicalGroup(2, [117, 271, 381], 2)  # 右平面

    gmsh.model.addPhysicalGroup(2, [33, 55, 77, 99, 121, 143, 165, 187, 209, 231, 253, 275, 297, 319, 341, 363, 385], 3)  # 后平面
    gmsh.model.addPhysicalGroup(2, [25, 47, 69, 91, 113, 135, 157, 179, 201, 223, 245, 267, 289, 311, 333, 355, 377], 4)  # 前平面

    gmsh.model.addPhysicalGroup(2, [302, 324, 346, 368, 390], 5)  # 底平面
