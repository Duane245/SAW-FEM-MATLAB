...
def build_mesh(config):
    """SAW_3D_Hex9_LTPML: Gmsh 模型构建(从 MATLAB 仓库 .py 复制并函数化)。"""
    gmsh.initialize()

    gmsh.model.add("SAW_3D_Hex9_LTPML")
    ...
    h3 = config.layers[1].thickness_um
    h4 = config.layers[2].thickness_um
    ...
    NN = max(1, math.ceil(d / .17 * 0.2 / lc - 1e-12))
    NN1 = max(1, math.ceil(3 * h1 / 0.6 * 0.2 / lc - 1e-12))
    NN2 = max(1, math.ceil(3 * h2 / 0.5 * 0.2 / lc - 1e-12))
    NN3 = max(1, math.ceil(5 * h3 / 1.0 * 0.2 / lc - 1e-12))
    NN4 = max(1, math.ceil(30 * h4 / 6.0 * 0.2 / lc - 1e-12))

    NN5 = max(1, math.ceil(10 * h5 / 2.0 * 0.2 / lc - 1e-12))
