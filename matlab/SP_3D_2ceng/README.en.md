...
**Self-contained 2.5D periodic-cell SAW resonator simulation — 2-layer (LT substrate + Si dielectric)**
...
A self-contained MATLAB demo of a **2.5D periodic-cell** SAW resonator using 27-node hexahedral (Hex27) high-order elements: a single IDT period with Bloch periodic boundaries on the left/right and front/back, equivalent to a finite-aperture infinite IDT array. Stack: **LiTaO₃ + Si + Al**; complex-coordinate-stretched PML at the bottom.
...
The 251-point sweep takes **≈ 10 min+** on a typical workstation (single process, no Parallel Computing Toolbox required). Mesh has ≈ 8 700 Hex27 nodes; memory scales with mesh size.
...
| Stack (substrate → top) | LiTaO₃ + Si + Al |
...
python mesh/SP_3D_2ceng.py
# This writes mesh/SP_3D_2ceng.m, which the driver loads via
#   SP_3D_2ceng;   % inside Solve3DSAW.m
```

If Gmsh / Python are not available on the run machine, pre-generate `SP_3D_2ceng.m` elsewhere and copy it into `mesh/`.
