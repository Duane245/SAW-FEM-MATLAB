...
**自包含的 2.5D 有限器件 SAW 谐振器仿真 —— LT + Al + SiO₂ + Poly-Si**
...
自包含的 MATLAB 算例,模拟 2.5D **有限器件**(FP)SAW 谐振器:21 周期多指 IDT + 反射栅 + 27 节点六面体(Hex27)高阶单元;左右两侧采用悬浮电势端部边界,前后施加 Bloch 周期边界以等效模拟无限大孔径。叠层结构:**LiTaO₃ + Al + SiO₂ + Poly-Si**;左 / 右 / 下方布置复坐标拉伸 PML 吸收外行波。
...
401 频点扫频在典型工作站上耗时 **数小时**(单进程,无需 Parallel Computing Toolbox);Hex27 节点数 ≈ 207 K,内存峰值受网格规模影响。
...
| 项 | 值 |
|---|---|
| 叠层结构(衬底 → 上) | LiTaO₃ + Al + SiO₂ + Poly-Si |
| 电极 | Al(21 周期多指 IDT) |
| 周期(pitch) | 1.0 µm |
...
# 该脚本输出 mesh/FP_3D_3ceng.m, 驱动脚本 Solve3DSAW.m 通过
#   FP_3D_3ceng;   % 调用该 .m 加载网格
```

如果运行机器上没有 Gmsh / Python,也可以在其它地方预先生成好 `FP_3D_3ceng.m` 再拷贝到 `mesh/` 目录。
