...
**Self-contained 2.5D periodic-cell SAW resonator simulation — 4-layer (LT + SiO₂ + Poly-Si + Si)**
...
A self-contained MATLAB demo of a **2.5D periodic-cell** SAW resonator using 27-node hexahedral (Hex27) high-order elements: a single IDT period with Bloch periodic boundaries on the left/right and front/back, equivalent to a finite-aperture infinite IDT array. Stack: **LiTaO₃ + SiO₂ + Poly-Si + Si + Al**; complex-coordinate-stretched PML at the bottom.
...
The 301-point sweep takes **≈ 12 min+** on a typical workstation (single process, no Parallel Computing Toolbox required). Mesh has ≈ 10 300 Hex27 nodes; memory scales with mesh size.
...
| Stack (substrate → top) | LiTaO₃ + SiO₂ + Poly-Si + Si + Al |
...
python mesh/SP_3D_4ceng.py
# This writes mesh/SP_3D_4ceng.m, which the driver loads via
#   SP_3D_4ceng;   % inside Solve3DSAW.m
```

If Gmsh / Python are not available on the run machine, pre-generate `SP_3D_4ceng.m` elsewhere and copy it into `mesh/`.
