...
**Self-contained 2.5D finite-device SAW resonator simulation — LN + Al**
...
A 401-point sweep takes **several hours** on a typical workstation (single process, no Parallel Computing Toolbox required); Hex27 node count ≈ 274 K.
...
| Sweep range | 1.80 – 2.20 GHz |
| Number of frequency points | 401 |
| Mesh node count | ≈ 274 K |
...
# This script produces mesh/FP_3D_1ceng.m, which the driver loads via
#   FP_3D_1ceng;
```

If the run machine lacks Gmsh / Python, generate `FP_3D_1ceng.m` elsewhere and copy it into `mesh/`.
