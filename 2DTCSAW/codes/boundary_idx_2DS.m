function Boundary = boundary_idx_2DS(gamma1_2,gamma1_L,gamma1_R,gamma1_zheng,gamma1_fu, ...
    nodeCoordinates,numberNodes,GDofu)
%BOUNDARY_IDX_2DS 周期单元（SP）二维平面分析的边界自由度索引。
%   - 力学：底面固定 + 左右 Bloch 周期；
%   - 电学：信号电极 / 接地电极 + 左右周期。
%% 左右边界去掉底边界
...
%% 边界条件（平面分析，每节点 2 个位移自由度）
% 固体力学
Boundary.machine.period_L = [gamma1_L; gamma1_L+numberNodes];
Boundary.machine.period_R = [gamma1_R; gamma1_R+numberNodes];
